...
const runtimeRoute = 'app/(child)/worlds.tsx';
if (!fs.existsSync(runtimeRoute)) fail('Missing required route file: app/(child)/worlds.tsx');
pass('Story World route file exists: app/(child)/worlds.tsx');
...
const allowedChanged = new Set([runtimeRoute, 'scripts/validate-story-world-visual-transformation-v1.mjs']);
for (const file of changedFiles) {
  if (!allowedChanged.has(file)) fail(`Changed file outside story-world scope: ${file}`);
}
pass('Only allowed runtime route file changed (plus validator script)');

const code = fs.readFileSync(runtimeRoute, 'utf8');
if (!code.includes('router.push(') || !/href:\s*'\/world\//.test(code)) fail('Existing navigation into story/world cards is not preserved');
pass('Existing navigation into story/world cards is preserved');
...
if (/prototype-only|screen\s*4[2-8][0-9]/i.test(code)) fail('Prototype-only screen references detected in Story World runtime route');
...
if (/\bLuvlu\b/i.test(code)) {
  ...
  pass('Luvlu governance respected in Story World usage');
} else pass('Luvlu not used in this route');

console.log(`ℹ️ Checked routes: ${discoveredRoutes.sort().join(', ')}`);
console.log('validate-story-world-visual-transformation-v1: PASS');
