...
const forbiddenStandalone=['fully accessible','guaranteed accessible','certified accessible'];
...
const wordRegex=(phrase)=>new RegExp(`\\b${escapeRegex(phrase)}\\b`,'g');
const wcagRegex=/\bWCAG compliant\b/g;
const notWcagRegex=/\bnot WCAG compliant\b/g;
const hasInvalidWcagClaim=(text)=>{
  const wcagMatches=text.match(wcagRegex)??[];
  const allowedMatches=text.match(notWcagRegex)??[];
  return wcagMatches.length!==allowedMatches.length;
};
const validateForbiddenClaims=(text, context)=>{
  for(const phrase of forbiddenStandalone){
    if(wordRegex(phrase).test(text)) fail(`Forbidden phrase in ${context}: ${phrase}`);
  }
  if(hasInvalidWcagClaim(text)) fail(`Forbidden phrase in ${context}: WCAG compliant`);
};
...
validateForbiddenClaims(doc, 'QA doc');
...
for(const f of surfaces){const t=fs.readFileSync(f,'utf8'); validateForbiddenClaims(t, f);}
