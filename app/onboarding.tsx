...
function OnboardingScreenContent() {
  ...
  const isEditing = !!getOnboardingState().profile;

  return (<SafeAreaView style={styles.screen}><View style={styles.card}><Text style={styles.stepCount}>Step {stepIndex + 1} of {steps.length}</Text><Text style={styles.title}>{current.title}</Text><Text style={styles.subtitle}>{current.subtitle}</Text><View style={styles.content}>{current.render()}</View><View style={styles.row}><TouchableOpacity accessibilityRole='button' accessibilityLabel='Go to previous onboarding step' disabled={stepIndex === 0} onPress={() => setStepIndex((s) => Math.max(0, s - 1))} style={[styles.button, styles.secondaryButton, stepIndex === 0 && styles.disabled]}><Text style={styles.secondaryText}>Back</Text></TouchableOpacity><TouchableOpacity accessibilityRole='button' accessibilityLabel={isLast ? 'Finish onboarding' : 'Go to next onboarding step'} onPress={async () => { if (isLast) { await completeOnboarding(profile); router.replace('/(child)/today'); return; } setStepIndex((s) => s + 1); }} style={[styles.button, styles.primaryButton]}><Text style={styles.primaryText}>{isLast ? (isEditing ? 'Save & Return to Child Home' : 'Enter Child Home') : 'Next'}</Text></TouchableOpacity></View><Link href='/(parent)/dashboard' style={styles.parentLink} accessibilityRole='link' accessibilityLabel='Go to Parent Dashboard'>Go to Parent Dashboard</Link></View></SafeAreaView>);
}

function ChoiceList<T extends string>({ items, selected, onPick }: { items: T[]; selected: T; onPick: (item: T) => void }) {
  return <View style={{ gap: 10 }}>{items.map((item) => <TouchableOpacity key={item} accessibilityRole='button' accessibilityLabel={`Select ${item}`} onPress={() => onPick(item)} style={[styles.choice, selected === item && styles.choiceSelected]}><Text style={[styles.choiceText, selected === item && styles.choiceTextSelected]}>{item}</Text></TouchableOpacity>)}</View>;
}
