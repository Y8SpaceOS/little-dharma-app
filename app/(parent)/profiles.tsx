...
export default function ParentProfilesGateScreen() {
  ...
  return <SafeAreaView style={styles.screen}><View style={styles.card}><Text style={styles.kicker}>For parents</Text><Text style={styles.title}>Grown-up area</Text><Text style={styles.sub}>This is a gentle local gate, not secure authentication.</Text><TextInput value={answer} onChangeText={setAnswer} placeholder='Parent word' style={styles.input} accessibilityLabel='Parent word entry' accessibilityHint='Enter the local parent word to open parent space' /><TouchableOpacity onPress={() => pass && router.replace('/(parent)/dashboard')} style={[styles.btn, !pass && styles.disabled]} disabled={!pass} accessibilityRole='button' accessibilityLabel='Open Parent Space' accessibilityState={{ disabled: !pass }}><Text style={styles.btnText}>Open Parent Space</Text></TouchableOpacity><Text style={styles.help}>Local-only placeholder check for this build. It does not create an account or provide security.</Text><Link href='/(child)/today' style={styles.link} accessibilityRole='link' accessibilityLabel='Back to Child Home'>Back to child world</Link></View></SafeAreaView>;
}
const styles = StyleSheet.create({screen:{flex:1,backgroundColor:'#F3F8FF',justifyContent:'center',padding:20},card:{backgroundColor:'#FFFCF6',borderRadius:24,padding:18,gap:10},kicker:{fontSize:13,fontWeight:'800',color:'#2E628F'},title:{fontSize:32,fontWeight:'900',color:'#5D3A1C'},sub:{fontSize:15,lineHeight:21,color:'#5A4A3D'},input:{backgroundColor:'#fff',borderRadius:12,padding:12,minHeight:48},btn:{backgroundColor:'#4D7AA8',padding:14,borderRadius:12,alignItems:'center',minHeight:48,justifyContent:'center'},btnText:{color:'#fff',fontWeight:'800'},disabled:{opacity:.45},help:{fontSize:12,color:'#6F5E52'},link:{textAlign:'center',fontWeight:'700',color:'#2E628F'}});
